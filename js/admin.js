...
import { collection, addDoc, doc, getDoc, updateDoc, deleteDoc, query, orderBy, onSnapshot, serverTimestamp } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
function initPanel(user){
  ...
  // stany edycji
  let editMenuId = null, editEntryId = null, editHelpId = null, selectedEntryIdForTTS = null;

  // ====== CKEDITOR ======
  function getEditorHtml(){
    try { if (window.CKEDITOR && CKEDITOR.instances.contentInput) return CKEDITOR.instances.contentInput.getData(); }
    catch(e){}
    return contentInput?.value || '';
  }
  function setEditorHtml(html=''){
    try { if (window.CKEDITOR && CKEDITOR.instances.contentInput) CKEDITOR.instances.contentInput.setData(html);
      else if (contentInput) contentInput.value = html; } catch(e){}
  }
  function initCk(){
    if (!window.CKEDITOR || !contentInput || CKEDITOR.instances.contentInput) return;
    const ed = CKEDITOR.replace('contentInput', {
      height: 280,
      removeButtons: '',
  ...
  // uruchom gdy DOM i gdy CKEditor się doczyta
  document.addEventListener('DOMContentLoaded', initCk);
  if (window.CKEDITOR) initCk(); else {
    // jeśli CDN dociąży się po czasie
    const ckReady = setInterval(() => {
      if (window.CKEDITOR) { clearInterval(ckReady); initCk(); }
    }, 300);
    setTimeout(()=>clearInterval(ckReady), 10000);
  }
  ...
  function updateLivePreview(){
    ...
    if (liveMeta) {
      const who = authorInput?.value || 'Autor';
      liveMeta.textContent = `${who} • ${new Date().toLocaleString('pl-PL')}`;
    }
  ...
  menuForm?.addEventListener('submit', async ev=>{
    ...
    try{
      ...
      addMenuBtn.innerHTML = '<i class="fa-solid fa-plus"></i> Dodaj';
      cancelMenuEditBtn.style.display='none';
      showTemp(menuMsg, 'Zapisano');
    }catch(e){ console.error(e); showTemp(menuMsg,'Błąd',false); }
  });
  ...
  function renderMenu(list=[]){
    if(!menuListContainer) return; menuListContainer.innerHTML='';
    list.forEach(it=>{
      const div = document.createElement('div'); div.className='list-item';
      div.innerHTML = `
        <div><div class="row"><div>
          <div class="entry-title" style="font-weight:700">${escapeHtml(it.text)}</div>
          <div class="muted-small">${escapeHtml(it.url)} • ${it.order}</div>
        </div></div></div>
        <div class="row">
          <button class="ghost small" data-action="edit" data-id="${it.id}"><i class="fa-solid fa-pen"></i></button>
          <button class="ghost small" data-action="del" data-id="${it.id}"><i class="fa-solid fa-trash"></i></button>
        </div>`;
    ...
    menuListContainer.querySelectorAll('button').forEach(btn=>btn.addEventListener('click', async ev=>{
      ...
      if (ev.currentTarget.dataset.action==='edit'){
        const d = (await getDoc(doc(db,'menu',id))).data();
        menuText.value=d.text; menuUrl.value=d.url; menuOrder.value=d.order;
        editMenuId=id; addMenuBtn.innerHTML='<i class="fa-solid fa-save"></i> Zapisz';
        cancelMenuEditBtn.style.display='inline-block';
  ...
  function populateSectionSelect(menuItems=[]){
    ...
    sectionSelect.innerHTML=''; filterSection.innerHTML='<option value="">Wszystkie sekcje</option>';
    const add = (txt)=>{ const opt = document.createElement('option'); opt.value=txt; opt.textContent=txt; sectionSelect.appendChild(opt); filterSection.appendChild(opt.cloneNode(true)); };
    if (!menuItems.length){ add('Kronika'); } else { menuItems.forEach(m=>add(m.text)); }
    sectionSelect.value = prev || sectionSelect.value;
  }
  onSnapshot(query(collection(db,'menu'), orderBy('order')), snap=>{
    const items = snap.docs.map(d=>({id:d.id, ...d.data()}));
    renderMenu(items); populateSectionSelect(items);
  });
  ...
  helpForm?.addEventListener('submit', async ev=>{
    ...
    try{
      ...
      helpForm.reset(); editHelpId=null;
      addHelpBtn.innerHTML='<i class="fa-solid fa-plus"></i> Dodaj Ośrodek';
      cancelHelpEditBtn.style.display='none';
      showTemp(helpMsg,'Zapisano');
    }catch(e){ console.error(e); showTemp(helpMsg,'Błąd',false); }
  });
  cancelHelpEditBtn?.addEventListener('click', ()=>{
    helpForm.reset(); editHelpId=null;
    addHelpBtn.innerHTML='<i class="fa-solid fa-plus"></i> Dodaj Ośrodek';
    cancelHelpEditBtn.style.display='none';
  });
  function renderHelp(list=[]){
    if(!helpListContainer) return; helpListContainer.innerHTML='';
    list.forEach(it=>{
      const div = document.createElement('div'); div.className='list-item';
      div.innerHTML = `
        <div><div style="font-weight:700">${escapeHtml(it.name)}</div>
          <div class="muted-small">${escapeHtml(it.woj)} | ${escapeHtml(it.address)}</div>
        </div>
        <div class="row">
          <button class="ghost small" data-action="edit" data-id="${it.id}"><i class="fa-solid fa-pen"></i></button>
          <button class="ghost small" data-action="del" data-id="${it.id}"><i class="fa-solid fa-trash"></i></button>
    ...
    helpListContainer.querySelectorAll('button').forEach(btn=>btn.addEventListener('click', async ev=>{
      const id = ev.currentTarget.dataset.id;
      if (ev.currentTarget.dataset.action==='edit'){
        const d = (await getDoc(doc(db,'help',id))).data();
      ...
      } else if (confirm('Na pewno usunąć?')) { await deleteDoc(doc(db,'help',id)); }
    }));
  }
  ...
  function updateDraftBadge(){
    ...
    try{
      ...
      draftBadge.textContent = ts ? `Wersja robocza: ${new Date(ts).toLocaleString('pl-PL')}` : 'Wersja robocza: —';
    }catch(e){ draftBadge.textContent='Wersja robocza: —'; }
  }
  loadDraft();
  ...
  entryForm?.addEventListener('submit', async ev=>{
    ...
    try{
        ...
        if (uploadPreview){
          ...
          uploadPreview.innerHTML = `
            <div style="display:flex;gap:10px;align-items:center">
              <div id="entryUploadThumb"></div>
              <div style="flex:1">
                <div class="muted-small">Wysyłanie: ${escapeHtml(f.name)}</div>
                <progress id="entryUploadProgress" value="0" max="100" style="width:100%"></progress>
              </div>
            </div>`;
          if (f.type.startsWith('image/')){
            const img = document.createElement('img');
            img.src = URL.createObjectURL(f); img.className='thumb';
            uploadPreview.querySelector('#entryUploadThumb').appendChild(img);
          }
        }
        await new Promise((res,rej)=>task.on('state_changed', s=>{
          const p = Math.round((s.bytesTransferred/s.totalBytes)*100);
          const bar = document.getElementById('entryUploadProgress'); if (bar) bar.value=p;
        }, rej, res));
      ...
      if (editEntryId) await updateDoc(doc(db,'entries',editEntryId), payload);
      else await addDoc(collection(db,'entries'), { ...payload, createdAt: serverTimestamp() });
      ...
      updateLivePreview(); clearDraft(); showTemp(formMsg,'Zapisano');
    }catch(e){ console.error(e); showTemp(formMsg,'Błąd zapisu',false); }
    finally{ publishBtn.disabled=false; }
  });

  cancelEntryEditBtn?.addEventListener('click', ()=>{
    editEntryId=null; entryForm?.reset(); setEditorHtml('');
    cancelEntryEditBtn.style.display='none';
    publishBtn.innerHTML='<i class="fa-solid fa-floppy-disk"></i> Opublikuj';
    updateLivePreview();
  });

  // ====== LISTA WPISÓW (z "Czytaj więcej") ======
  let entriesCache = [];
  function renderEntries(list=[]){
    ...
    arr.forEach(e=>{
      ...
      div.innerHTML = `
        <div class="list-item-content">
          ${attHtml}
          <div>
            <div class="entry-title" style="font-weight:700">${titleHtml}</div>
            <div class="muted-small">${escapeHtml(e.section||'')} • ${escapeHtml(e.author||'')} • ${date}</div>

            <div class="entry-excerpt">${escapeHtml(excerpt)}</div>
            <div class="entry-full">${e.text || ''}</div>
            ${plain.length>220 ? `<button class="ghost small toggle-more" data-more="1">Czytaj więcej</button>` : ''}
          </div>
        </div>
        <div class="row">
          <button class="ghost small" data-action="edit" data-id="${e.id}"><i class="fa-solid fa-pen"></i></button>
          <button class="ghost small danger" data-action="del" data-id="${e.id}"><i class="fa-solid fa-trash"></i></button>
        </div>`;

      entriesList.appendChild(div);
    });

    // Zaznaczanie do lektora
    entriesList.querySelectorAll('.list-item').forEach(item=>{
      item.addEventListener('click', (ev)=>{
        if (ev.target.closest('button')) return; // nie łap klików na przyciskach
    ...
    // Czytaj więcej / mniej
    entriesList.querySelectorAll('.toggle-more').forEach(btn=>{
      btn.addEventListener('click', (ev)=>{
        ev.stopPropagation();
        const box = btn.closest('.list-item');
        const ex = box.querySelector('.entry-excerpt');
        const full = box.querySelector('.entry-full');
        const expanded = btn.dataset.more === '0';
        if (expanded){
          // schowaj pełny
    ...
    // edycja / usuwanie
    entriesList.querySelectorAll('button[data-action]').forEach(btn=>{
      btn.addEventListener('click', async ev=>{
        ...
        if (act==='edit'){
          try{
            ...
            const d = snap.data();
            editEntryId = id;
            sectionSelect.value = d.section || sectionSelect.value;
  ...
  }, err=>console.error('entries onSnapshot', err));
  ...
  // ====== ISKIERKI / PLAYLISTA / GALERIA ======
  sparkForm?.addEventListener('submit', async ev=>{
    ev.preventDefault();
    const qtxt = (sparkInput?.value||'').trim(); if (!qtxt) return;
    try{ await addDoc(collection(db,'sparks'), { quote:qtxt, createdAt: serverTimestamp() }); sparkInput.value=''; }
    catch(e){ console.error(e); }
  });
  playlistForm?.addEventListener('submit', async ev=>{
    ev.preventDefault();
    const t=(songTitle?.value||'').trim(), l=(songLink?.value||'').trim(); if (!t||!l) return;
    try{ await addDoc(collection(db,'playlist'), { title:t, link:l, createdAt: serverTimestamp() }); songTitle.value=''; songLink.value=''; }
    catch(e){ console.error(e); }
  });
  galleryForm?.addEventListener('submit', async ev=>{
    ...
    task.on('state_changed', s=>{
      ...
      galleryForm.reset(); galleryProgressBar.value=0;
    });
  });
  function renderSparks(list=[]){
    ...
    list.forEach(s=>{
      ...
      el.innerHTML = `<div><i class="fa-solid fa-star"></i> ${escapeHtml(s.quote)}</div>
      <div class="row"><button class="ghost small danger" data-id="${s.id}"><i class="fa-solid fa-trash"></i></button></div>`;
      sparksList.appendChild(el);
    });
    sparksList.querySelectorAll('button').forEach(btn=>btn.addEventListener('click', async ev=>{
      const id = ev.currentTarget.dataset.id; if (confirm('Usuń?')) await deleteDoc(doc(db,'sparks',id));
    }));
  }
  function renderPlaylist(list=[]){
    ...
    list.forEach(s=>{
      const el = document.createElement('div'); el.className='list-item';
      el.innerHTML = `<div><div style="font-weight:700">${escapeHtml(s.title)}</div><div class="muted-small">${escapeHtml(s.link)}</div></div>
      <div class="row"><button class="ghost small danger" data-id="${s.id}"><i class="fa-solid fa-trash"></i></button></div>`;
      playlistList.appendChild(el);
    });
    playlistList.querySelectorAll('button').forEach(btn=>btn.addEventListener('click', async ev=>{
      const id = ev.currentTarget.dataset.id; if (confirm('Usuń?')) await deleteDoc(doc(db,'playlist',id));
    }));
  }
  function renderGallery(list=[]){
    ...
    list.forEach(g=>{
      ...
      el.innerHTML = `<div style="display:flex;gap:10px;align-items:center">
        <img src="${g.url}" class="thumb" alt=""><div style="font-weight:700">${escapeHtml(g.desc)}</div></div>
        <div class="row"><button class="ghost small danger" data-id="${g.id}" data-path="${g.meta?.path}"><i class="fa-solid fa-trash"></i></button></div>`;
    ...
    galleryList.querySelectorAll('button').forEach(btn=>btn.addEventListener('click', async ev=>{
      ...
      try{ if (path) await deleteObject(sref(storage, path)).catch(()=>{}); await deleteDoc(doc(db,'gallery', id)); }
      catch(e){ console.error(e); }
    }));
  }
  ...
  ttsListenBtn?.addEventListener('click', async ()=>{
    ...
    try{
      const snap = await getDoc(doc(db,'entries',selectedEntryIdForTTS));
      if (!snap.exists()) return alert('Wpis nie istnieje');
      const d = snap.data();
      const txt = stripHtml((d.title ? d.title.replace(/<[^>]*>?/gm, '') + '. ' : '') + (d.text || '')).trim();
