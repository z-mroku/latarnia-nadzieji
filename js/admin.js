...
async function initPanel(user){
  ...
  let entriesCache = [];
// CZĘŚĆ 2/5: TWOJA ORYGINALNA LOGIKA (CKEDITOR 4, IKONY, MENU, POMOC)
  ...
  function updateLivePreview(){
    ...
    if (uploadPreview){ if (!uploadPreview.dataset.locked) { uploadPreview.innerHTML = ''; const f = attachInput?.files?.[0]; if (f){ const url = URL.createObjectURL(f); if (f.type.startsWith('image/')){ const img = document.createElement('img'); img.src = url; img.className = 'thumb'; uploadPreview.appendChild(img); } else { uploadPreview.textContent = f.name; } } } }
  }
  ...
  menuForm?.addEventListener('submit', async ev => { ev.preventDefault(); const data = { text: menuText.value.trim(), url: menuUrl.value.trim(), order: Number(menuOrder.value) || 0, createdAt: serverTimestamp() }; if (!data.text || !data.url) return; try { if (editMenuId) { await updateDoc(doc(db, 'menu', editMenuId), data); } else { await addDoc(collection(db, 'menu'), data); } menuForm.reset(); editMenuId = null; addMenuBtn.innerHTML = '<i class="fa-solid fa-plus"></i> Dodaj'; cancelMenuEditBtn.style.display='none'; showTemp(menuMsg, 'Zapisano'); } catch (e) { showTemp(menuMsg, 'Błąd', false); console.error(e); } });
  ...
  cancelMenuEditBtn?.addEventListener('click', () => { helpForm.reset(); editHelpId = null; addHelpBtn.innerHTML='<i class="fa-solid fa-plus"></i> Dodaj Ośrodek'; cancelHelpEditBtn.style.display='none'; });
    ...
    try {
      ...
      if (editEntryData) {
          ...
          if (originalSection !== newSection) {
              ...
              await setDoc(newDocRef, { ...docToMoveData, ...payload });
...
// CZĘŚĆ 4/5: LISTA WPISÓW I AKCJE
  ...
  function renderEntries(list=[]){
    ...
    entriesList.querySelectorAll('button').forEach(btn=>{
      btn.addEventListener('click', async ev=>{
        ...
        if (act === 'edit') {
          editEntryData = { id: id, section: entry.section };
          ...
          if (entry.attachment?.url && uploadPreview) { uploadPreview.innerHTML = `<img src="${entry.attachment.url}" class="thumb" alt="Podgląd załącznika">`; } else { uploadPreview.innerHTML = ''; }
