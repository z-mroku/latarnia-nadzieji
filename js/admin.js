...
// CZĘŚŚĆ 1/5: INICJALIZACJA I DEFINICJE
...
// CZĘŚŚĆ 2/5: TWOJA ORYGINALNA LOGIKA (CKEDITOR 4, IKONY, MENU, POMOC)
  ...
  menuForm?.addEventListener('submit', async ev => {
    ...
    try {
      if (editMenuId) {
        const ref = doc(db, 'menu', editMenuId);
    ...
    menuListContainer.querySelectorAll('button').forEach(btn => btn.addEventListener('click', async ev => {
      ...
      if (act === 'edit') {
        try {
          ...
          const d = snap.data();
          menuText.value = d.text || '';
          menuUrl.value = d.url || '';
          menuOrder.value = d.order ?? 0;
          editMenuId = id;
          addMenuBtn.innerHTML = '<i class="fa-solid fa-save"></i> Zapisz';
  ...
  function populateSectionSelect(menuItems = []) {
    ...
    // zachowaj poprzedni wybór jeśli istnieje, inaczej ustaw domyślnie 'Kronika' tylko jeśli select jest pusty
    if (prev && Array.from(sectionSelect.options).some(o => o.value === prev)) sectionSelect.value = prev;
    else if (sectionSelect.options.length && !sectionSelect.value) sectionSelect.value = 'Kronika';
  }
  ...
  helpForm?.addEventListener('submit', async ev => {
    ...
    try {
      if (editHelpId) {
        await updateDoc(doc(db, 'help', editHelpId), data);
      } else {
  ...
  // POPRAWKA: powinno nasłuchiwać cancelHelpEditBtn, nie cancelMenuEditBtn
    ...
    helpListContainer.querySelectorAll('button').forEach(btn => btn.addEventListener('click', async ev => {
      ...
      if (action === 'edit') {
        try {
          ...
          editHelpId = id;
          addHelpBtn.innerHTML = '<i class="fa-solid fa-save"></i> Zapisz Zmiany';
          cancelHelpEditBtn.style.display = 'inline-block';
        } catch (e) { console.error(e); }
      } else {
...
// CZĘŚŚĆ 3/5: WERSJE ROBOCZE I FORMULARZ WPISÓW Z POPRAWKĄ
  ...
  entryForm?.addEventListener('submit', async ev => {
    ...
    try {
      ...
      if (f) {
        ...
        // POPRAWKA: użyj uploadBytes (promise) zamiast await na UploadTask
      ...
      if (editEntryData) {
        ...
        if (originalSection !== newSection) {
          ...
          if (!oldSnap.exists()) {
            // jeśli nie ma starego - zapisz bez usuwania
            ...
            // zachowaj createdAt jeśli istniało
      ...
      entryForm.reset(); setEditorHtml(''); editEntryData = null; publishBtn.innerHTML = '<i class="fa-solid fa-floppy-disk"></i> Opublikuj'; cancelEntryEditBtn.style.display = 'none'; updateLivePreview(); clearDraft();
...
// CZĘŚŚĆ 4/5: LISTA WPISÓW I AKCJE
  ...
  function renderEntries(list = []) {
    ...
    // zaznaczanie elementu i informacja o TTS
...
// CZĘŚŚĆ 5/5: MAŁE MODUŁY, MODAL I ZAKOŃCZENIE
  ...
  galleryForm?.addEventListener('submit', async ev => {
    ...
    task.on('state_changed', s => { const p = Math.round((s.bytesTransferred / s.totalBytes) * 100); if (galleryProgressBar) galleryProgressBar.value = p; }, console.error, async () => { const url = await getDownloadURL(r); await addDoc(collection(db, 'gallery'), { url, desc, meta: { path }, createdAt: serverTimestamp() }); galleryForm.reset(); if (galleryProgressBar) galleryProgressBar.value = 0; });
  });
