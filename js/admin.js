// /js/admin.js - WERSJA PRAWDZIWIE FINALNA, KOMPLETNA I POPRAWIONA
...
async function initPanel(user){
  ...
  function populateSectionSelect(menuItems=[]){
    ...
    menuItems.filter(m => !m.url.startsWith('http') && m.url.toLowerCase().includes('sekcja.html')).forEach(m=>{
  ...
  galleryForm?.addEventListener('submit', async ev => {
      ...
      if (editGalleryId) {
          ...
          const entryRef = doc(db, 'gallery', editGalleryId);
          try {
              await updateDoc(entryRef, { desc: desc, updatedAt: serverTimestamp() });
              galleryForm.reset();
              editGalleryId = null;
      ...
      if (!file) {
          showTemp(formMsg, 'Wybierz plik', false);
          return;
      }
      addGalleryBtn.disabled = true;
      ...
      uploadTask.on('state_changed', 
          ...
          (error) => {
              ...
              showTemp(formMsg, 'Błąd uploadu', false);
              addGalleryBtn.disabled = false;
          }, 
          async () => {
              ...
              showTemp(formMsg, 'Zdjęcie dodane');
              addGalleryBtn.disabled = false;
          }
      );
  });
  ...
  function renderSparks(list=[]){
      ...
      list.forEach(s => {
          ...
          el.innerHTML = `<span>${escapeHtml(s.quote)}</span><button data-id="${s.id}">Usuń</button>`;
          el.querySelector('button').onclick = async () => { if(confirm('Usunąć?')) await deleteDoc(doc(db, 'sparks', s.id)); };
  ...
  function renderPlaylist(list=[]){
      ...
      list.forEach(s => {
          const el = document.createElement('div');
          el.className = 'list-item';
          el.innerHTML = `<div><strong>${escapeHtml(s.title)}</strong><br><small>${escapeHtml(s.link)}</small></div><button data-id="${s.id}">Usuń</button>`;
          el.querySelector('button').onclick = async () => { if(confirm('Usunąć?')) await deleteDoc(doc(db, 'playlist', s.id)); };
  ...
  function renderGallery(list=[]){
      ...
      list.forEach(g => {
          const el = document.createElement('div');
          el.className = 'list-item';
          el.innerHTML = `<img src="${g.url}" class="thumb" alt="${escapeHtml(g.desc)}"><span>${escapeHtml(g.desc)}</span><button data-id="${g.id}" data-path="${g.meta.path}">Usuń</button>`;
