...
async function initPanel(user) {
  ...
  function getEditorHtml() {
    try {
      if (window.CKEDITOR && window.CKEDITOR.instances && window.CKEDITOR.instances.contentInput) {
        return window.CKEDITOR.instances.contentInput.getData();
      }
    } catch (e) { console.warn('getEditorHtml err', e); }
    return contentInput?.value || '';
  }
  function setEditorHtml(html = '') {
    try {
      if (window.CKEDITOR && window.CKEDITOR.instances && window.CKEDITOR.instances.contentInput) {
        window.CKEDITOR.instances.contentInput.setData(html);
      } else if (contentInput) {
        contentInput.value = html;
      }
    } catch (e) { console.warn('setEditorHtml err', e); }
  }
  function initCk() {
    try {
  ...
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initCk);
  else initCk();
  ...
  menuForm?.addEventListener('submit', async ev=>{
    ...
    try {
      ...
      menuForm.reset(); editMenuId=null;
      if (addMenuBtn) addMenuBtn.innerHTML = '<i class="fa-solid fa-plus"></i> Dodaj';
      if (cancelMenuEditBtn) cancelMenuEditBtn.style.display='none';
      showTemp(menuMsg,'Zapisano');
    } catch(e){ showTemp(menuMsg,'Błąd',false); console.error(e); }
  });
  cancelMenuEditBtn?.addEventListener('click', ()=>{ menuForm.reset(); editMenuId=null; if (addMenuBtn) addMenuBtn.innerHTML = '<i class="fa-solid fa-plus"></i> Dodaj'; if (cancelMenuEditBtn) cancelMenuEditBtn.style.display='none'; });
  ...
  function renderMenu(list=[]){
    ...
    list.forEach(it=>{
      ...
      div.innerHTML = `<div><div style="font-weight:700">${escapeHtml(it.text)}</div><div class="muted-small">${escapeHtml(it.url)} • ${it.order}</div></div>
        <div class="row"><button class="ghost small" data-action="edit" data-id="${it.id}"><i class="fa-solid fa-pen"></i></button><button class="ghost small danger" data-action="del" data-id="${it.id}"><i class="fa-solid fa-trash"></i></button></div>`;
    ...
    menuListContainer.querySelectorAll('button').forEach(btn=>btn.addEventListener('click', async ev=>{
      ...
      if (act === 'edit') {
        try {
          ...
          editMenuId = id; if (addMenuBtn) addMenuBtn.innerHTML = '<i class="fa-solid fa-save"></i> Zapisz'; if (cancelMenuEditBtn) cancelMenuEditBtn.style.display='inline-block';
  ...
  function populateSectionSelect(menuItems=[]){
    ...
    menuItems.forEach(m=>{
      const opt = document.createElement('option'); opt.value = m.text; opt.textContent = m.text;
    ...
    else if (sectionSelect.options.length && !sectionSelect.value) {
      const kron = Array.from(sectionSelect.options).find(o=>o.value==='Kronika');
      if (kron) sectionSelect.value = 'Kronika';
    }
  }
  ...
  helpForm?.addEventListener('submit', async ev=>{
    ...
    try {
      ...
      helpForm.reset(); editHelpId=null; if (addHelpBtn) addHelpBtn.innerHTML = '<i class="fa-solid fa-plus"></i> Dodaj Ośrodek'; if (cancelHelpEditBtn) cancelHelpEditBtn.style.display='none';
      showTemp(helpMsg,'Zapisano');
    } catch(e){ showTemp(helpMsg,'Błąd',false); console.error(e); }
  });
  cancelHelpEditBtn?.addEventListener('click', ()=>{ helpForm.reset(); editHelpId=null; if (addHelpBtn) addHelpBtn.innerHTML = '<i class="fa-solid fa-plus"></i> Dodaj Ośrodek'; if (cancelHelpEditBtn) cancelHelpEditBtn.style.display='none'; });
  ...
  function renderHelp(list=[]){
    ...
    helpListContainer.querySelectorAll('button').forEach(btn=>btn.addEventListener('click', async ev=>{
      ...
      if (action === 'edit') {
        ...
        try { await deleteDoc(doc(db,'help',id)); } catch(e){ console.error(e); }
      }
    }));
  }
  ...
  function saveDraft(){
    ...
    try { localStorage.setItem(DRAFT_KEY, JSON.stringify(draft)); updateDraftBadge(); } catch(e){}
  }
  function loadDraft(){
    try {
  ...
  entryForm?.addEventListener('submit', async ev=>{
    ...
    try {
      ...
      if (f) {
        ...
        // prefer uploadBytes for small/simple cases, support resumable if available
        try {
          await uploadBytes(sRef, f);
        } catch (e) {
          // fallback to resumable
        ...
        payload.attachment = { url, meta: { path } };
      }
      ...
      showTemp(formMsg,'Błąd zapisu', false); console.error(e);
  ...
  function renderEntries(list=[]){
    ...
    if (q) arr = arr.filter(x => {
      const t = stripHtml(x.title||'').toLowerCase();
      const s = stripHtml(x.text||'').toLowerCase();
      ...
      btn.addEventListener('click', async ev=>{
        ...
        if (act === 'edit') {
          try {
            ...
            const d = snap.data();
            editEntryData = { id, section: entry.section, createdAt: d.createdAt, attachment: d.attachment };
  ...
  function renderSparks(list=[]){
    ...
    list.forEach(s=>{ const el = document.createElement('div'); el.className='list-item'; el.innerHTML = `<div><i class="fa-solid fa-star"></i> ${escapeHtml(s.quote)}</div><div class="row"><button class="ghost small danger" data-id="${s.id}"><i class="fa-solid fa-trash"></i></button></div>`; sparksList.appendChild(el); });
  ...
  galleryForm?.addEventListener('submit', async ev=>{
    ...
    try {
      ...
      // ensure section doc exists
      const sectionName = 'Galeria';
      const secRef = doc(db,'sekcje', sectionName);
      const secSnap = await getDoc(secRef);
      if (!secSnap.exists()) await setDoc(secRef, { name: "Galeria", order: 99, createdAt: serverTimestamp() });

      await addDoc(collection(db,'sekcje', sectionName, 'entries'), {
      ...
      galleryForm.reset(); galleryProgressBar && (galleryProgressBar.value = 0);
  ...
  onSnapshot(query(collection(db,'sparks'), orderBy('createdAt','desc')), ()=>{}); // placeholder to keep realtime init stable
  ...
  function renderGallery(list=[]){
    ...
    list.forEach(g=>{
      ...
      el.innerHTML = `<div style="display:flex;gap:10px;align-items:center">${g.type === 'video' ? `<div class="entry-list-thumb" style="background:#000;color:#fff;display:flex;align-items:center;justify-content:center;">🎥</div>` : `<img src="${g.url}" class="thumb" alt="">`}<div><div style="font-weight:700">${escapeHtml(g.title || g.desc || '')}</div><div class="muted-small">${escapeHtml(g.type || 'image')}</div></div></div><div class="row"><button class="ghost small" data-action="edit" data-id="${g.id}" title="Edytuj opis"><i class="fa-solid fa-pen"></i></button><button class="ghost small danger" data-action="del" data-id="${g.id}" data-path="${g.storagePath || g.meta?.path}" title="Usuń"><i class="fa-solid fa-trash"></i></button></div>`;
    ...
    galleryList.querySelectorAll('button').forEach(btn=> btn.addEventListener('click', async ev=>{
      ...
      if (action === 'edit') {
        ...
        editGalleryId = id; galleryUpload.disabled = true;
        const addBtn = galleryForm.querySelector('button[type="submit"]'); if (addBtn) addBtn.innerHTML = '<i class="fa-solid fa-save"></i> Zapisz zmiany';
        galleryDesc.focus();
      } else if (action === 'del') {
  ...
  function showModalWithData(id, d){
    ...
    modalEditBtn.onclick = async ()=>{
      ...
      try {
        ...
        editEntryData = { id, section: d.section, attachment: docData.attachment, createdAt: docData.createdAt };
