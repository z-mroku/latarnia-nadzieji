// ========================================
//  LATARNIA NADZIEI - ADMIN PANEL (FULL VERSION)
// ========================================
...
const firebaseConfig = {
  ...
  // ✅ Prawidłowy bucket (firebasestorage.app)
...
// Style dynamiczne dla panelu admina
const dynamicAdminStyles = document.createElement('style');
dynamicAdminStyles.textContent = `
    .list-item.item-pending { border-left: 4px solid #e94560; background: rgba(233, 69, 96, 0.05); }
    .list-item.item-approved { border-left: 4px solid #4caf50; }
    ...
    .status-badge { font-size: 0.75rem; padding: 2px 6px; border-radius: 4px; font-weight: bold; text-transform: uppercase; margin-left: 8px; }
    .status-badge.pending { background: #e94560; color: white; }
    .status-badge.approved { background: #4caf50; color: white; }
    /* Styl aktywnych przycisków filtrów */
    button.filter-btn.active { background-color: #ffd97b !important; color: #000 !important; border-color: #ffd97b !important; opacity: 1 !important; }
    button.filter-btn { opacity: 0.6; transition: all 0.2s; }
`;
...
function runAdminPanel(user) {
    ...
    async function initPanel(user) {
        ...
        const state = {
            editors: {},
            editId: { menu: null, help: null, spark: null, playlist: null, note: null, letter: null, audio: null },
    ...
    async function initEditor(state) {
        ...
        try {
            ...
            if ($('helpDesc')) state.editors.help = await ClassicEditor.create($('helpDesc'), simpleEditorConfig);
            if ($('noteContent')) state.editors.notes = await ClassicEditor.create($('noteContent'), simpleEditorConfig);
        } catch (error) {
    ...
    function initEntryListAndFilter(state) {
        ...
        onSnapshot(query(collectionGroup(db, 'entries'), orderBy('createdAt', 'desc')), (snapshot) => {
            ...
            console.error("Błąd w onSnapshot dla 'entries':", error);
            // toast("Brak uprawnień do wczytania listy wpisów!", false);
        });
        onSnapshot(query(collection(db, 'menu'), orderBy('order')), (snapshot) => {
            state.menuItems = snapshot.docs.map(d => d.data());
            populateSectionSelect(state.entry.cache, state.menuItems);
        }, (error) => {
            console.error("Błąd w onSnapshot dla 'menu':", error);
            // toast("Brak uprawnień do wczytania menu.", false);
        });
    }
    ...
    function loadModule(name, element, state) {
        ...
        const moduleInitializers = {
            ...
            audio: initAudio // ✅ DODANE: Rejestracja modułu Audio
        };
    ...
    // ========================================
    // MODUŁ MODERACJI WSPARCIA (Z FILTRAMI)
    // ========================================
    function initSupportModeration(state) {
        ...
        let supportCache = [];
        let currentFilter = 'pending'; // Domyślnie Oczekujące

        // Pobieramy przyciski filtrów
        const btnPending = $('filterSupportPending');
        const btnApproved = $('filterSupportApproved');
        const btnAll = $('filterSupportAll');

        const updateButtons = () => {
            [btnPending, btnApproved, btnAll].forEach(b => {
            ...
            else if (currentFilter === 'approved') activeBtn = btnApproved;
        ...
        const renderList = () => {
            ...
            filtered.forEach(item => {
                ...
                const statusBadge = item.isApproved 
                    ? '<span class="status-badge approved">WIDOCZNY</span>' 
                    : '<span class="status-badge pending">OCZEKUJE</span>';

                div.innerHTML = `
                    <div style="flex-grow: 1;">
                        <p><strong>${escapeHtml(item.name)}:</strong> ${escapeHtml(item.text)} ${statusBadge}</p>
                        <div class="muted-small">${date} • ${item.likes || 0} ❤️</div>
                    </div>
                    <div class="row" style="flex-shrink: 0; align-items: flex-start; gap: 0.5rem;">
                        <button class="ghost small ${item.isApproved ? 'danger' : 'primary'}" data-action="toggle">
                            ${item.isApproved ? '<i class="fa-solid fa-eye-slash"></i> Ukryj' : '<i class="fa-solid fa-check"></i> Zatwierdź'}
                        </button>
        ...
        listContainer.addEventListener('click', async (e) => {
            ...
            const item = supportCache.find(i => i.id === itemId);
            const docRef = doc(db, 'support_stories', itemId);

            try {
                if (action === 'toggle') {
                    await updateDoc(docRef, { isApproved: !item.isApproved });
                    toast(item.isApproved ? 'Wpis ukryty.' : 'Wpis zatwierdzony.');
                } else if (action === 'delete') {
                    if (await confirmAction({title: "Potwierdź usunięcie", text: "Czy na pewno chcesz trwale usunąć ten wpis?", confirmText: "Usuń"})) {
        ...
        if(btnPending) btnPending.onclick = () => { currentFilter = 'pending'; updateButtons(); renderList(); };
        if(btnApproved) btnApproved.onclick = () => { currentFilter = 'approved'; updateButtons(); renderList(); };
        if(btnAll) btnAll.onclick = () => { currentFilter = 'all'; updateButtons(); renderList(); };
        
        updateButtons();
    }

    // ========================================
    // MODUŁ MODERACJI KOMENTARZY (Z FILTRAMI)
    // ========================================
    function initCommentModeration(state) {
        ...
        const btnPending = $('filterCommentsPending');
        const btnApproved = $('filterCommentsApproved');
        const btnAll = $('filterCommentsAll');
        ...
        const renderList = () => {
            ...
            filtered.forEach(item => {
                ...
                div.innerHTML = `
                    <div style="flex-grow: 1;">
                        <p><strong>${escapeHtml(item.author)}:</strong> ${escapeHtml(item.text)} ${statusBadge}</p>
                        <div class="muted-small comment-context">Wątek ID: ${item.storyId} • ${date}</div>
                    </div>
                    <div class="row" style="flex-shrink: 0; align-items: flex-start; gap: 0.5rem;">
                        <button class="ghost small ${item.isApproved ? 'danger' : 'primary'}" data-action="toggle">
                            ${item.isApproved ? '<i class="fa-solid fa-eye-slash"></i>' : '<i class="fa-solid fa-check"></i>'}
                        </button>
                        <button class="ghost small danger" data-action="delete" title="Usuń trwale"><i class="fa-solid fa-trash"></i> Usuń</button>
                    </div>
                `;
        ...
        onSnapshot(q, (snapshot) => {
            commentCache = snapshot.docs.map(d => ({
                id: d.id,
                path: d.ref.path,
        ...
        listContainer.addEventListener('click', async (e) => {
            ...
            if (!button) return;
            const itemEl = e.target.closest('.list-item');
            const docPath = itemEl?.dataset.path;
            const action = button.dataset.action;
            if (!docPath) return;
            
            const docRef = doc(db, docPath);
            const item = commentCache.find(c => c.path === docPath);

            try {
                if (action === 'toggle') {
                    await updateDoc(docRef, { isApproved: !item.isApproved });
                    toast(item.isApproved ? 'Komentarz ukryty.' : 'Komentarz zatwierdzony.');
                } else if (action === 'delete') {
                    if (await confirmAction({title: "Usuń", text: "Usunąć komentarz?", confirmText: "Usuń"})) {
                        await deleteDoc(docRef);
    ...
    // ========================================
    // 🎙️ MODUŁ AUDIO (GŁOS LATARNI)
    // ========================================
    function initAudio(state) {
        const form = $('audioForm');
        const listContainer = $('audioList');
        const inputs = {
            title: $('audioTitle'),
        ...
        const progressWrap = $('audioProgressWrap');
        const progressBar = $('audioProgressBar');
        const cancelBtn = $('cancelAudioEditBtn');

        if (!form) return;

        let audioCache = [];

        // 1. Pobieranie i wyświetlanie listy
        onSnapshot(query(collection(db, 'audio_tracks'), orderBy('category'), orderBy('order')), (snapshot) => {
            audioCache = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
            
            if (audioCache.length === 0) {
                listContainer.innerHTML = '<div class="list-empty-state">Brak nagrań. Dodaj coś, żeby przemówić.</div>';
                return;
            }

            // Grupujemy po kategoriach dla czytelności
            const grouped = audioCache.reduce((acc, item) => {
                    ...
                    div.innerHTML = `
                        <div style="flex-grow:1;">
                            <div style="font-weight:700;">
                                <i class="fa-solid fa-play-circle"></i> ${escapeHtml(item.title)}
                            </div>
                            <div class="muted-small">${escapeHtml(item.description || '')}</div>
                        </div>
                        <div class="row">
                            <button class="ghost small danger" data-action="del-audio" data-id="${item.id}" title="Usuń"><i class="fa-solid fa-trash"></i></button>
                        </div>
                    `;
                    listContainer.appendChild(div);
                });
        ...
        // 2. Usuwanie
        listContainer.addEventListener('click', async e => {
            const btn = e.target.closest('button[data-action="del-audio"]');
            if (!btn) return;
            const id = btn.dataset.id;
            const item = audioCache.find(i => i.id === id);
            
            if (await confirmAction({ text: `Usunąć nagranie "${item.title}"?` })) {
                try {
                    // Usuwamy plik z Storage
                    if (item.filePath) {
                        const fileRef = sref(storage, item.filePath);
                        await deleteObject(fileRef).catch(err => console.warn("Plik nie istniał:", err));
                    }
                    // Usuwamy wpis z Bazy
                    await deleteDoc(doc(db, 'audio_tracks', id));
                    toast('Nagranie usunięte.');
                } catch (err) {
                    console.error(err);
                    toast('Błąd usuwania.', false);
                }
            }
        });
        ...
        form.addEventListener('submit', async e => {
            ...
            const order = Number(inputs.order.value) || 1;
            const description = inputs.desc.value.trim();

            if (!title || !category) return toast('Tytuł i kategoria są wymagane.', false);
            if (!file && !state.editId.audio) return toast('Musisz wybrać plik MP3.', false);
            ...
            try {
                ...
                // Jeśli wybrano plik - wysyłamy do Storage
                if (file) {
                    // Ścieżka: audio_tracks/kategoria/timestamp_nazwa.mp3
                    filePath = `audio_tracks/${category}/${Date.now()}_${file.name.replace(/\s/g, '_')}`;
                ...
                await addDoc(collection(db, 'audio_tracks'), docData);
                toast('Nagranie dodane pomyślnie!');
                ...
                console.error(err);
                toast('Błąd wysyłania: ' + err.message, false);
            } finally {
                submitBtn.disabled = false;
            }
        });
